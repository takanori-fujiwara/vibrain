    ...
    : QDialog(parent)
{
    ...
    connect(matrixUsedForMdsBtnGroup, SIGNAL(buttonClicked(QAbstractButton*)), this, SLOT(sendSignalMatrixUsedForMdsBtnChanged(QAbstractButton*)));

    ///
    /// Distance Calculation Method
    ///
    QGroupBox *mdsDistMethodGroupBox = new QGroupBox("MDS Distance Calculation Method");
    ...
    QStringList mdsDistMethods = QStringList({"Euclidean distance", "Cosine distance", "Correlation"});
    ...
    connect(mdsDistMethodBtnGroup, SIGNAL(buttonClicked(QAbstractButton*)), this, SLOT(sendSignalMdsDistMethodBtnChanged(QAbstractButton*)));
    ...
    for (int i = 0; i < categoryKeys.size(); ++i) {
        ...
        QVector<QString> valuesOfCategories = scannedDataset->getValuesOfCategories()[categoryKeys.at(i)];
        for (int j = 0; j < valuesOfCategories.size(); ++j) {
            ...
            connect(checkBox, SIGNAL(clicked(bool)), this, SLOT(sendSignalFilterChanged()));
        }
    }
    ...
    for (int i = 0; i < numberKeys.size(); ++i) {
        ...
        connect(comboBox, SIGNAL(currentTextChanged(QString)), this, SLOT(sendSignalFilterChanged()));
        connect(lineEdit, SIGNAL(textChanged(QString)), this, SLOT(sendSignalFilterChanged()));
    }
    ...
    connect(applyFilteringBtn, SIGNAL(clicked(bool)), this, SLOT(sendSignalApplyFilteringClicked()));
    connect(recalcMdsBtn, SIGNAL(clicked(bool)), this, SLOT(sendSignalRecalcMdsClicked()));
    ...
    connect(pointColorBtnGroup, SIGNAL(buttonClicked(QAbstractButton*)), this, SLOT(sendSignalPointColorBtnChanged(QAbstractButton*)));
    ...
    connect(pointShapeBtnGroup, SIGNAL(buttonClicked(QAbstractButton*)), this, SLOT(sendSignalPointShapeBtnChanged(QAbstractButton*)));
    ...
    for (int i = 0; i < numberKeys.size(); ++i) {
        // QVector<float> minMaxOfNumbers = scannedDataset->getMinAndMaxOfNumbers()[numberKeys.at(i)];
        QRadioButton *radio = new QRadioButton(numberKeys.at(i)
    ...
    connect(pointSizeBtnGroup, SIGNAL(buttonClicked(QAbstractButton*)), this, SLOT(sendSignalPointSizeBtnChanged(QAbstractButton*)));
...
void SettingPanel::sendSignalFilterChanged()
{
    ...
    foreach (QGroupBox* keyGroupBox, filteringCategoryGroupBox->findChildren<QGroupBox*>()) {
        ...
        statusOfCategoryFilterCheckBoxes.insert(keyGroupBox->objectName(), categoryValuesAndChecks);
    }
    ...
    foreach (QGroupBox* keyGroupBox, filteringNumberGroupBox->findChildren<QGroupBox*>()) {
        ...
        statusOfNumberFilterOperatorsAndValues.insert(keyGroupBox->objectName(), operatorAndValues);
    }

    emit filterChanged(statusOfCategoryFilterCheckBoxes, statusOfNumberFilterOperatorsAndValues);
}
