...
Style::Style(const QString &dirPath) {
  ...
  selectedFillColor = QColor(150, 200, 150, 100);
...
bool Style::loadStyle(const QString &dirPath) {
  ...
  if (!loadColors(colormap, corColormapFilePath))
    return false;
  if (!loadColors(uncertaintyColormap, uncColormapFilePath))
    return false;
  if (!loadColors(pointColors, pointColorsFilePath))
    return false;
  if (!loadColors(roiColors, roiColorsFilePath))
    return false;
  if (!loadShapes(pointShapes, pointShapesFilePath))
    return false;
...
bool Style::loadColors(QVector<QColor> &targetColors, const QString &filePath) {
  ...
  while (!ts.atEnd()) {
    ...
    if (elements.size() < 4) {
      qWarning() << "there are incorrect number of values in color file (need "
                    "4 numbers) in line: "
                 << lineNum;
      return false;
    }

    targetColors.push_back(
        QColor(elements.at(0).toInt(), elements.at(1).toInt(),
               elements.at(2).toInt(), elements.at(3).toInt()));
...
bool Style::loadShapes(QVector<int> &targetShapes, const QString &filePath) {
  ...
  while (!ts.atEnd()) {
    ...
    if (elements.size() < 1) {
      qWarning() << "there are incorrect number of values in shape file (need "
                    "1 number) in line: "
                 << lineNum;
      return false;
    }
