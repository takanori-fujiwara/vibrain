...
EdgeBundleConnections::EdgeBundleConnections(Rois* rois, DetailedMatrix* detailedMatrix)
{
    params();

    for (int i = 0; i < rois->getNumOfRois(); ++i) {
        QVector3D* anode;
        anode = new QVector3D(rois->getRoiAt(i)->getX(),
                              rois->getRoiAt(i)->getY(),
...
double EdgeBundleConnections::vis_c(EdgeBundleEdge* ep, EdgeBundleEdge* eq)
{
    ...
    return qMax( 1.0-2.0*(float)((pm-im).length()) / (float)((i0-i1).length()), 0.0); // modified
}
