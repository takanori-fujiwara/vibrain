...
ThresholdSettingDialog::ThresholdSettingDialog(DetailedMatrix *detailedMatrix,
                                               QWidget *parent) {
  ...
  lowerThresLineEdit->setText(
      QString::number(detailedMatrix->getLowerThresVal()));
  upperThresLineEdit->setText(
      QString::number(detailedMatrix->getUpperThresVal()));
  ...
  connect(cancelBtn, &QPushButton::clicked, this,
          &ThresholdSettingDialog::handleCancelBtn);
  connect(okBtn, &QPushButton::clicked, this,
          &ThresholdSettingDialog::handleOkBtn);
...
void ThresholdSettingDialog::handleCancelBtn() { close(); }
...
void ThresholdSettingDialog::handleOkBtn() {
  ...
  if (!(thresMin <= lowerThres)) {
    errorMsgLabel->setText(
        "Lower thres must be equal or larger than thres min");
    return;
  }

  if (!(lowerThres <= upperThres)) {
    errorMsgLabel->setText(
        "Upper thres must be equal or larger than lower thres");
    return;
  }

  if (!(upperThres <= thresMax)) {
    errorMsgLabel->setText(
        "Thres max must be equal or larger than upper thres");
    return;
  }
