...
EdgeBundleEdge::EdgeBundleEdge(QVector3D fn, QVector3D tn) {
  this->fn = fn;
  this->tn = tn;
  points << fn << tn;
  QVector3D nv(0, 0, 0);
...
void EdgeBundleEdge::subdivide(int newp) {
...
double EdgeBundleEdge::segLength(int n) {
...
bool EdgeBundleEdge::flip(EdgeBundleEdge *other) {
...
double EdgeBundleEdge::length() { return (fn - tn).length(); }

void EdgeBundleEdge::applyForces() {
