...
void ComparisonView::drawMdsErrors() {
  foreach (ScannedData *sd, scannedDataset->getScannedDataset()) {
    if (sd->getVisible()) {
      ...
      float size = 0.1 * qMax(0.0, (sd->getMdsError() - 0.0)) *
                   qMax(0.0, (sd->getMdsError() - 0.0)) * 0.02 / scale;
      drawRadialColCircle(sd->getPos().x(), sd->getPos().y(), size,
                          style->mdsErrorColorCenter, style->mdsErrorColorPeri);
    }
  }
}
...
void ComparisonView::drawPoints() {
  ...
  foreach (ScannedData *sd, scannedDataset->getScannedDataset()) {
    if (sd->getVisible() && sd->getFilteredOut()) {
      ...
      drawPoint(style->pointShapes.at(sd->getShapeGroup()), sd->getPos().x(),
                sd->getPos().y(), sd->getSize() / scale, fillColor.darker(300),
                style->strokeColor, 0.002 / scale);
    }
  }
  ...
  foreach (ScannedData *sd, scannedDataset->getScannedDataset()) {
    if (sd->getVisible() && !sd->getFilteredOut() && !sd->getSelected()) {
      drawPoint(style->pointShapes.at(sd->getShapeGroup()), sd->getPos().x(),
                sd->getPos().y(), sd->getSize() / scale,
                style->pointColors.at(sd->getColorGroup()), style->strokeColor,
                0.002 / scale);
    }
  }
  ...
  foreach (ScannedData *sd, scannedDataset->getScannedDataset()) {
    if (sd->getVisible() && !sd->getFilteredOut() && sd->getSelected()) {
      drawPoint(style->pointShapes.at(sd->getShapeGroup()), sd->getPos().x(),
                sd->getPos().y(), sd->getSize() / scale,
                style->pointColors.at(sd->getColorGroup()),
                style->selectedStrokeColor, 0.004 / scale);
    }
  }
}
...
void ComparisonView::drawLinesFromMatrixToPoint() {
  ...
  foreach (ScannedData *sd, scannedDataset->getScannedDataset()) {
    if (sd->getVisible() && sd->getMatrixDisplayed() && sd->getFilteredOut()) {
      drawLineFromMatrixToPoint(sd->getMatrixPos(), sd->getPos(), 0.003 / scale,
                                style->lineFromMatrixToPointColor);
    }
  }
...
void ComparisonView::drawMatrices(QString matrixKey) {
  ...
  foreach (ScannedData *sd, scannedDataset->getScannedDataset()) {
    if (sd->getVisible() && sd->getMatrixDisplayed() && sd->getFilteredOut()) {
      ...
      drawMatrix(matrixKey, sd->getMatrixAt(matrixKey), sd->getMatrixPos().x(),
                 sd->getMatrixPos().y(), glyphSize / scale, order,
                 style->colormap, style->strokeColor);
      isShowColorMap = true;
    }
  }
  ...
  foreach (ScannedData *sd, scannedDataset->getScannedDataset()) {
    if (sd->getVisible() && sd->getMatrixDisplayed() && !sd->getFilteredOut() &&
      ...
      drawMatrix(matrixKey, sd->getMatrixAt(matrixKey), sd->getMatrixPos().x(),
                 sd->getMatrixPos().y(), glyphSize / scale, order,
                 style->colormap, style->selectedStrokeColor, 3.0);
      isShowColorMap = true;
    }
  }
}
...
void ComparisonView::drawLasso() {
  glColor4f(
      style->selectedStrokeColor.redF(), style->selectedStrokeColor.greenF(),
      style->selectedStrokeColor.blueF(), style->selectedStrokeColor.alphaF());
...
void ComparisonView::paintLegends() {
  ...
  if (scannedDataset->getCategoryForColorGroups() != "none") {
    QVector<QString> labels = scannedDataset->getValuesOfCategories()
                                  [scannedDataset->getCategoryForColorGroups()];
    ...
    for (int i = 0; i < labels.size(); ++i) {
      ...
      painter.drawRect(legendStartX, legendStartY + fontH / 4, fontH - 2,
                       fontH / 2);
      painter.setPen(Qt::white);
      painter.drawText(legendStartX + fontH, legendStartY, labelW, fontH,
                       Qt::AlignVCenter | Qt::AlignLeft, labels.at(i));
      legendStartY += fontH;
      if (labelW > colorLegendsWidth)
        colorLegendsWidth = labelW;
    }
  ...
  if (scannedDataset->getCategoryForShapeGroups() != "none") {
    QVector<QString> labels = scannedDataset->getValuesOfCategories()
                                  [scannedDataset->getCategoryForShapeGroups()];
    legendStartY -= fontH * labels.size();
    for (int i = 0; i < labels.size(); ++i) {
      int labelW = labels.at(i).size() * fontW;
      ...
      painter.setPen(Qt::white);
      painter.drawPolygon(
          genLegendShapePoints(legendStartX, legendStartY + fontH / 4,
                               fontH - 2, style->pointShapes.at(i)));
      painter.drawText(legendStartX + fontH, legendStartY, labelW, fontH,
                       Qt::AlignVCenter | Qt::AlignLeft, labels.at(i));
      legendStartY += fontH;
    }
  ...
  if (isShowColorMap) {
    ...
    painter.setBrush(Qt::white);
    painter.setPen(Qt::white);
    painter.drawText(x - fontW * 2, y - fontH, fontW * 4, fontH,
                     Qt::AlignVCenter | Qt::AlignCenter,
                     QString::number(colorMapMinVal, 'g', 3));
    painter.drawText(x + w - fontW * 2, y - fontH, fontW * 4, fontH,
                     Qt::AlignVCenter | Qt::AlignCenter,
                     QString::number(colorMapMaxVal, 'g', 3));
  }
  ...
  if (isShowMdsErrors) {
    ...
    painter.drawText(x - fontW * 2, y - fontH, fontW * 4, fontH,
                     Qt::AlignVCenter | Qt::AlignCenter,
                     QString::number(mdsErrorColorMapMinVal, 'g', 3));
    painter.drawText(x + w - fontW * 2, y - fontH, fontW * 4, fontH,
                     Qt::AlignVCenter | Qt::AlignCenter,
                     QString::number(mdsErrorColorMapMaxVal, 'g', 3));
  }
...
void ComparisonView::paintPopup() {
  ...
  if (hoveredScannedDataId >= 0) {
    ...
    QString longestText = "";
    QStringList keys;
    keys = scannedDataset->getScannedDataAt(hoveredScannedDataId)
               ->getSubIds()
               .keys();
    foreach (QString key, keys) {
      QString text =
          key + ": " +
          QString::number(scannedDataset->getScannedDataAt(hoveredScannedDataId)
                              ->getSubIdAt(key));
      if (text.size() > longestText.size())
        longestText = text;
      info += text + "\n";
    }
    keys = scannedDataset->getScannedDataAt(hoveredScannedDataId)
    ...
    foreach (QString key, keys) {
      QString text = key + ": " +
                     scannedDataset->getScannedDataAt(hoveredScannedDataId)
                         ->getCategoryAt(key);
      if (text.size() > longestText.size())
        longestText = text;
      info += text + "\n";
    ...
    int numOfInfo = scannedDataset->getScannedDataAt(hoveredScannedDataId)
                        ->getNumOfSubIds() +
                    scannedDataset->getScannedDataAt(hoveredScannedDataId)
                        ->getNumOfCategories() +
                    scannedDataset->getScannedDataAt(hoveredScannedDataId)
...
QPolygonF ComparisonView::genLegendShapePoints(int x, int y, int size,
                                               int shapeType) {
  ...
  for (int i = 0; i <= numOfSides; i++) {
    points.push_back(
        QPointF((float)x + (float)size * 0.5 +
                    ((float)size * 0.5 *
                     cos(-halfPi + (float)i * twicePi / (float)numOfSides)),
                (float)y + (float)size * 0.5 +
                    ((float)size * 0.5 *
                     sin(-halfPi + (float)i * twicePi / (float)numOfSides))));
  }

  return QPolygonF(points);
}

void ComparisonView::drawRadialColCircle(float x, float y, float radius,
                                         QColor colorCenter, QColor colorPeri) {
  int numOfSides = 30; //# of vertex of triangles used to draw circle
  GLfloat twicePi = 2.0f * M_PI;

  glBegin(GL_TRIANGLE_FAN);
  glColor4d(colorCenter.redF(), colorCenter.greenF(), colorCenter.blueF(),
            colorCenter.alphaF());
  glVertex2f(x, y);

  glColor4d(colorPeri.redF(), colorPeri.greenF(), colorPeri.blueF(),
            colorPeri.alphaF());
  for (int i = 0; i <= numOfSides; i++) {
    glVertex2f(x + (radius * cos((float)i * twicePi / (float)numOfSides)),
               y + (radius * sin((float)i * twicePi / (float)numOfSides)));
  }
  glEnd();
}

void ComparisonView::drawPoint(int shapeType, float x, float y, float size,
  ...
  glColor4d(strokeColor.redF(), strokeColor.greenF(), strokeColor.blueF(),
            strokeColor.alphaF());
  glBegin(GL_TRIANGLE_FAN);
  glVertex2f(x, y); // center of circle
  for (int i = 0; i <= numOfSides; i++) {
    glVertex2f(x + ((size + strokeWidth) *
                    cos(halfPi + (float)i * twicePi / (float)numOfSides)),
               y + ((size + strokeWidth) *
                    sin(halfPi + (float)i * twicePi / (float)numOfSides)));
  }
  ...
  glColor4d(fillColor.redF(), fillColor.greenF(), fillColor.blueF(),
            fillColor.alphaF());
  glBegin(GL_TRIANGLE_FAN);
  glVertex2f(x, y); // center of circle
  for (int i = 0; i <= numOfSides; i++) {
...
void ComparisonView::drawQuad(float x, float y, float w, float h,
                              QColor color) {
...
void ComparisonView::drawLineQuad(float x, float y, float w, float h,
...
void ComparisonView::drawMatrix(QString matrixKey,
                                std::vector<std::vector<float>> mat, float x,
                                float y, float size, QVector<int> orderOfMatrix,
                                QVector<QColor> colormap, QColor strokeColor,
                                float strokeWidth) {
  glPushMatrix();
  {
    ...
    for (int i = 0; i < displayedNumCol; ++i) {
      for (int j = 0; j < displayedNumCol; ++j) {
        ...
        if (i_ >= 0 && j_ >= 0) {
          ...
          float scaledVal =
              fmin(1.0, fmax(0.0, (val - colorMapMinVal) /
                                      (colorMapMaxVal - colorMapMinVal)));
...
void ComparisonView::drawLineFromMatrixToPoint(QPointF startPos, QPointF endPos,
                                               float width, QColor color) {
  ...
  glVertex2f(startPos.x() + normVec.dx() * width,
             startPos.y() + normVec.dy() * width);
  glVertex2f(startPos.x() - normVec.dx() * width,
             startPos.y() - normVec.dy() * width);
...
QPointF ComparisonView::qtPosToGlPos(QPoint qtPos) {
  float resultX =
      (-1.0 + 2.0 * (float)qtPos.x() / (float)this->width()) / scale -
      translateX;
  float resultY =
      -(-1.0 + 2.0 * (float)qtPos.y() / (float)this->height()) / scale -
      translateY;
  return QPointF(resultX, resultY);
}

bool ComparisonView::isInsidePolygon(
...
std::vector<GLfloat> ComparisonView::genCirclePoints(float cx, float cy,
                                                     float r, int numSplit) {
...
void ComparisonView::selectElement(std::vector<GLfloat> selectionAreaGlBuffer) {
  ...
  for (int i = scannedDataset->getScannedDataset().size() - 1; i >= 0; --i) {
    if (isInsidePolygon(scannedDataset->getScannedDataAt(i)->getPos(),
                        selectionAreaGlBuffer) ||
        isInsidePolygon(scannedDataset->getScannedDataAt(i)->getMatrixPos(),
                        selectionAreaGlBuffer)) {
...
void ComparisonView::selectMovingMatrixScannedDataId(
    std::vector<GLfloat> selectionAreaGlBuffer) {
  // reverse for, to select the point in front
  for (int i = scannedDataset->getScannedDataset().size() - 1; i >= 0; --i) {
    if (scannedDataset->getScannedDataAt(i)->getMatrixDisplayed() &&
        isInsidePolygon(scannedDataset->getScannedDataAt(i)->getMatrixPos(),
                        selectionAreaGlBuffer)) {
...
void ComparisonView::selectElementsInsideLasso() {
  for (int i = 0; i < scannedDataset->getScannedDataset().size(); ++i) {
    if (isInsidePolygon(scannedDataset->getScannedDataAt(i)->getPos(),
      ...
      // these are used for calc diff mat
      if (!selectedScannedDataIdsInOrder.contains(i))
        selectedScannedDataIdsInOrder.push_back(i);
    }
  }
...
void ComparisonView::mousePressEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    ...
    std::vector<GLfloat> circlePoints =
        genCirclePoints(eventPosF.x(), eventPosF.y(), 0.1 / scale);
...
void ComparisonView::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (event->buttons()) {
    if (movingMatrixScannedDataId >= 0) {
      scannedDataset->getScannedDataAt(movingMatrixScannedDataId)
          ->setMatrixPos(qtPosToGlPos(event->pos()));
    } else {
    ...
    QPointF eventPosF = qtPosToGlPos(event->pos());
    std::vector<GLfloat> circlePoints =
        genCirclePoints(eventPosF.x(), eventPosF.y(), 0.02 / scale);
...
void ComparisonView::mouseReleaseEvent(QMouseEvent *event) {
  ...
  if (event->button() == Qt::LeftButton) {
    if (lassoPoints.size() > 1) {
      ...
      if (mousePosDelta.manhattanLength() < 5 &&
          movingMatrixScannedDataId < 0) {
        // TODO: need to change the selection method using vbo to make more
        // accurate selection
        std::vector<GLfloat> circlePoints =
            genCirclePoints(eventPosF.x(), eventPosF.y(), 0.04 / scale);
...
void ComparisonView::showContextMenu(const QPoint &pos) {
  ...
  QAction actShowMatricesOfSelectedPoints("Show matrices on selected points",
                                          this);
  ...
  QAction actShowAveMatrixInDetailViews("Show averaged matrix in other views",
                                        this);
  QAction actShowDiffMatrixInDetailViews("Show diff matrix in other views",
                                         this);
  QAction actShowSdMatrixInDetailViews(
      "Show standard deviation matrix in other views", this);
  QAction actShowMaxMinMatrixInDetailViews(
      "Show max - min matrix in other views", this);
  ...
  QAction actChangeColorMapMinMax(
      "Change Correlation color map min and max values", this);

  if (isShowMdsErrors)
    actSwitchMdsError.setText("Hide MDS uncertainty");
  else
    actSwitchMdsError.setText("Show MDS uncertainty");

  connect(&actClearSelection, &QAction::triggered, this,
          &ComparisonView::clearSelection);
  connect(&actHideAllMatrices, &QAction::triggered, this,
          &ComparisonView::hideAllMatrices);
  connect(&actShowMatricesOfSelectedPoints, &QAction::triggered, this,
          &ComparisonView::showMatricesOfSelectedPoints);
  connect(&actHideMatricesOfSelectedPoints, &QAction::triggered, this,
          &ComparisonView::hideMatricesOfSelectedPoints);
  connect(&actShowMatrixInDetailViews, &QAction::triggered, this,
          &ComparisonView::showMatrixInDetailViews);
  connect(&actShowAveMatrixInDetailViews, &QAction::triggered, this,
          &ComparisonView::showAveMatrixInDetailViews);
  connect(&actShowDiffMatrixInDetailViews, &QAction::triggered, this,
          &ComparisonView::showDiffMatrixInDetailViews);
  connect(&actShowSdMatrixInDetailViews, &QAction::triggered, this,
          &ComparisonView::showSdMatrixInDetailViews);
  connect(&actShowMaxMinMatrixInDetailViews, &QAction::triggered, this,
          &ComparisonView::showMaxMinMatrixInDetailViews);
  connect(&actRecalcMds, &QAction::triggered, this, &ComparisonView::recalcMds);
  connect(&actSwitchMdsError, &QAction::triggered, this,
          &ComparisonView::switchShowMdsErrors);
  connect(&actChangeMdsErrorSettings, &QAction::triggered, this,
          &ComparisonView::changeMdsErrorSettings);
  connect(&actChangeColorMapMinMax, &QAction::triggered, this,
          &ComparisonView::changeColorMapMinMax);

  int numOfMatrixDisplayedIndices =
      scannedDataset->getMatrixDisplayedIndices().size();
  ...
  if (numOfSelectedIndices > 0)
    contextMenu.addAction(&actShowMatricesOfSelectedPoints);
  ...
  if (numOfSelectedIndices > 2)
    contextMenu.addAction(&actRecalcMds);
...
void ComparisonView::changeColorMapMinMax() {
  ...
  colorMapMinMaxDialog->setWindowTitle(
      "Correlation Colormap Min and Max Value Setting for Comparison View");
  ...
  connect(cancelBtn, &QPushButton::clicked, this,
          &ComparisonView::handleColorMapMinMaxDialogCancelBtn);
  connect(okBtn, &QPushButton::clicked, this,
          &ComparisonView::handleColorMapMinMaxDialogOkBtn);
...
void ComparisonView::changeMdsErrorSettings() {
  ...
  mdsErrorColorMapMinValLineEdit->setText(
      QString::number(mdsErrorColorMapMinVal));
  mdsErrorColorMapMaxValLineEdit = new QLineEdit;
  mdsErrorColorMapMaxValLineEdit->setText(
      QString::number(mdsErrorColorMapMaxVal));
  ratioNeighborForMdsErrorLineEdit = new QLineEdit;
  ratioNeighborForMdsErrorLineEdit->setText(
      QString::number(ratioNeighborForMdsError));
  ...
  connect(cancelBtn, &QPushButton::clicked, this,
          &ComparisonView::handleMdsErrorSettingDialogCancelBtn);
  connect(okBtn, &QPushButton::clicked, this,
          &ComparisonView::handleMdsErrorSettingDialogOkBtn);
...
void ComparisonView::updateMdsErrorVboAndCbo() {
  ...
  foreach (ScannedData *sd, scannedDataset->getScannedDataset()) {
    if (sd->getVisible()) {
      ...
      if (mdsError < minMdsError)
        minMdsError = mdsError;
      if (mdsError > maxMdsError)
        maxMdsError = mdsError;
      mdsErrors.push_back(mdsError);
      mdsPositions.push_back(sd->getPos());
    }
  }

  if (mdsErrorColorMapMinVal < 0)
    mdsErrorColorMapMinVal = minMdsError;
  if (mdsErrorColorMapMaxVal < 0)
    mdsErrorColorMapMaxVal = maxMdsError;
  ...
  if (mdsErrors.size() != 0 && mdsPositions.size() != 0) {
    ...
    for (int i = 0; i <= numDivXForMdsError; ++i) {
      ...
      for (int j = 0; j <= numDivYForMdsError; ++j) {
        ...
        float interpolatedVal = vi.calcWeightedAverageFromPos(
            mdsErrors, mdsPositions, pos,
            ratioNeighborForMdsError * mdsErrors.size());

        float scaledVal = 0.0;
        if (mdsErrorColorMapMaxVal - mdsErrorColorMapMinVal != 0) {
          scaledVal = qMax(
              0.0,
              qMin(1.0, (interpolatedVal - mdsErrorColorMapMinVal) /
                            (mdsErrorColorMapMaxVal - mdsErrorColorMapMinVal)));
        }

        QColor color = style->uncertaintyColormap.at(
            (int)(scaledVal * (style->uncertaintyColormap.size() - 1)));
...
void ComparisonView::setScanneDataset(ScannedDataset *scannedDataset) {
