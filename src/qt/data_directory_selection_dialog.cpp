...
DataDirectorySelectionDialog::DataDirectorySelectionDialog(QWidget *parent) {
  ...
  brainMeshDirLabel->setSizePolicy(QSizePolicy::Fixed,
  ...
  scannedDataDirLabel->setSizePolicy(QSizePolicy::Fixed,
                                     QSizePolicy::MinimumExpanding);
  styleDirLabel->setMinimumWidth(200);
  styleDirLabel->setSizePolicy(QSizePolicy::Fixed,
                               QSizePolicy::MinimumExpanding);
  ...
  connect(baseDataDirPushBtn, &QPushButton::clicked, this,
          &DataDirectorySelectionDialog::setBaseDataDir);
  connect(brainMeshDirPushBtn, &QPushButton::clicked, this,
          &DataDirectorySelectionDialog::setBrainMeshDir);
  connect(roiDirPushBtn, &QPushButton::clicked, this,
          &DataDirectorySelectionDialog::setRoiDir);
  connect(scannedDataDirPushBtn, &QPushButton::clicked, this,
          &DataDirectorySelectionDialog::setScannedDataDir);
  connect(styleDirPushBtn, &QPushButton::clicked, this,
          &DataDirectorySelectionDialog::setStyleDir);
  connect(cancelBtn, &QPushButton::clicked, this,
          &DataDirectorySelectionDialog::close);
  connect(okBtn, &QPushButton::clicked, this,
          &DataDirectorySelectionDialog::handleOk);
}
...
void DataDirectorySelectionDialog::setBrainMeshDir() {
  brainMeshDir =
      QFileDialog::getExistingDirectory(this, tr("Open Directory"), baseDataDir,
                                        QFileDialog::ShowDirsOnly |
                                            QFileDialog::DontResolveSymlinks) +
      "/";
  brainMeshDirLineEdit->setText(brainMeshDir);
...
void DataDirectorySelectionDialog::setRoiDir() {
  roiDir = QFileDialog::getExistingDirectory(
               this, tr("Open Directory"), baseDataDir,
...
void DataDirectorySelectionDialog::setStyleDir() {
  styleDir = QFileDialog::getExistingDirectory(
                 this, tr("Open Directory"), baseDataDir,
                 QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks) +
             "/";
  styleDirLineEdit->setText(styleDir);
...
void DataDirectorySelectionDialog::handleOk() {
  if (QDir(baseDataDir).exists() && QDir(brainMeshDir).exists() &&
      QDir(roiDir).exists() && QDir(scannedDataDir).exists() &&
      QDir(styleDir).exists()) {
    saveDirectories();
    emit dataDirecotriesChanged(baseDataDir, brainMeshDir, roiDir,
                                scannedDataDir, styleDir);
    this->close();
  } else {
    QMessageBox msgBox;
...
void DataDirectorySelectionDialog::saveDirectories() {
  ...
  settings.setValue("data_directory_selection_dialog/brainMeshDir",
                    brainMeshDir);
  settings.setValue("data_directory_selection_dialog/roiDir", roiDir);
  settings.setValue("data_directory_selection_dialog/scannedDataDir",
                    scannedDataDir);
...
void DataDirectorySelectionDialog::setCurrentSelectedDirecotries() {
  ...
  if (settings.value("data_directory_selection_dialog/baseDataDir").isValid()) {
    baseDataDirLineEdit->setText(
        settings.value("data_directory_selection_dialog/baseDataDir")
            .toString());
    baseDataDir = baseDataDirLineEdit->text();
  }
  if (settings.value("data_directory_selection_dialog/brainMeshDir")
          .isValid()) {
    brainMeshDirLineEdit->setText(
        settings.value("data_directory_selection_dialog/brainMeshDir")
            .toString());
    brainMeshDir = brainMeshDirLineEdit->text();
  }
  if (settings.value("data_directory_selection_dialog/roiDir").isValid()) {
    roiDirLineEdit->setText(
        settings.value("data_directory_selection_dialog/roiDir").toString());
    roiDir = roiDirLineEdit->text();
  }
  if (settings.value("data_directory_selection_dialog/scannedDataDir")
          .isValid()) {
    scannedDataDirLineEdit->setText(
        settings.value("data_directory_selection_dialog/scannedDataDir")
            .toString());
    scannedDataDir = scannedDataDirLineEdit->text();
  }
  if (settings.value("data_directory_selection_dialog/styleDir").isValid()) {
    styleDirLineEdit->setText(
        settings.value("data_directory_selection_dialog/styleDir").toString());
