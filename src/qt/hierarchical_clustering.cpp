...
void HierarchicalClustering::hclust(std::vector<std::vector<float>> &targetMat,
                                    QString hclustMethod,
  ...
  if (!QStringList({"single", "complete", "average", "centroid", "median",
                    "ward", "mcquitty"})
           .contains(hclustMethod)) {
  ...
  m_R["mat"] = rMat;
  m_R.parseEval("hclustResult <- hclust(dist(mat), method=\"" +
                hclustMethod.toStdString() + "\")");

  order = order.fromStdVector(
      Rcpp::as<std::vector<int>>(m_R.parseEval("hclustResult$order")));
  heights = heights.fromStdVector(
      Rcpp::as<std::vector<float>>(m_R.parseEval("hclustResult$height")));
  QVector<int> mergeHistoryCol1 = mergeHistoryCol1.fromStdVector(
      Rcpp::as<std::vector<int>>(m_R.parseEval("hclustResult$merge[,1]")));
  QVector<int> mergeHistoryCol2 = mergeHistoryCol2.fromStdVector(
      Rcpp::as<std::vector<int>>(m_R.parseEval("hclustResult$merge[,2]")));
  ...
  communityNumbers =
      communityNumbers.fromStdVector(Rcpp::as<std::vector<int>>(m_R.parseEval(
          "cutree(hclustResult, k=" + std::to_string(numOfCommunities) + ")")));
...
bool HierarchicalClustering::qPairSecondLessThan(QPair<int, int> pair1,
                                                 QPair<int, int> pair2) {
