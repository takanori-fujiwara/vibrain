...
bool ScannedData::loadScannedDataFile(const QString &filePath)
{
    ...
    while (!ts.atEnd()) {
        ...
        if (elements.size() < 3) {
            qWarning() << "there are incorrect number of values for loading spatial correction (need 3 elments) in line: " << lineNum;
            return false;
        }
        ...
        if (dataType == "id") {
            ...
            qWarning() << "there are unknown data type (you can use id,category,number, or matrix) in line: " << lineNum;
            return false;
        }
...
QMap<QString, std::vector<std::vector<float>> > ScannedData::getMatrices() { return matrices; }
...
std::vector<std::vector<float>> ScannedData::getMatrixAt(QString key) { return matrices[key]; }
float ScannedData::getMatrixElementAt(QString key, int row, int col) { return matrices[key].at(row).at(col); }
...
void ScannedData::setColorGroup(uint colorGroup) { this->colorGroup = colorGroup; }
void ScannedData::setShapeGroup(uint shapeGroup) { this->shapeGroup = shapeGroup; }
...
void ScannedData::setFilteredOut(bool filteredOut) { this->filteredOut = filteredOut; }
void ScannedData::setMatrixDisplayed(bool matrixDisplayed) { this->matrixDisplayed = matrixDisplayed; }
void ScannedData::setMatrixPos(QPointF matrixPos) { this->matrixPos = matrixPos; }
