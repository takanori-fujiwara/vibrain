...
bool BrainMesh::loadSpatialCorrection(const QString &filePath) {
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    qWarning() << "Couldn't open spatial correction file. file path:"
               << filePath;
    return false;
  }
...
std::vector<GLfloat> BrainMesh::getBrainMeshPositions() {
  return brainMeshPositions;
}
