    ...
    ui(new Ui::MainWindow)
{
    ...
    rPackageInstallMsgBox.setWindowFlags(Qt::WindowTitleHint | Qt::Dialog | Qt::CustomizeWindowHint | Qt::FramelessWindowHint);
    rPackageInstallMsgBox.setLayout(rPackageInstallMsgBoxLayout);
    rPackageInstallMsgBoxLayout->addWidget(rPackageInstallMsg);
    rPackageInstallMsgBox.setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding);
    rPackageInstallMsg->setText("Installing R packages...");
    QFutureWatcher<void> rPackageInstallWatcher;
    connect(&rPackageInstallWatcher, SIGNAL(finished()), &rPackageInstallMsgBox, SLOT(close()));
    QFuture<void> rPackageInstallFuture = QtConcurrent::run(this, &MainWindow::installRPackages);
    ...
    msgBox.setWindowFlags(Qt::WindowTitleHint | Qt::Dialog | Qt::CustomizeWindowHint | Qt::FramelessWindowHint);
...
void MainWindow::installRPackages()
{
    ...
    m_R.parseEval("if (!require(Rcpp)) { install.packages(\"Rcpp\", repos='http://cran.us.r-project.org') }");
    m_R.parseEval("if (!require(data.table)) { install.packages(\"data.table\", repos='http://cran.us.r-project.org') }");
    m_R.parseEval("if (!require(SnowballC)) { install.packages(\"SnowballC\", repos='http://cran.us.r-project.org') }");
    m_R.parseEval("if (!require(lsa)) { install.packages(\"lsa\", repos='http://cran.us.r-project.org') }");
    m_R.parseEval("if (!require(modMax)) { install.packages(\"modMax\", repos='http://cran.us.r-project.org') }");
}
...
void MainWindow::initWidgets()
{
    ...
    stackedWidgetL->setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed));
    stackedWidgetM1->setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed));
    stackedWidgetM2->setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed));
    stackedWidgetS->setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed));
    comboBoxForList->setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Expanding));
    stackedWidgetList->setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed));
...
void MainWindow::showDirSelectionDialog()
{
    ...
    connect(dirSelectionDialog, &DataDirectorySelectionDialog::dataDirecotriesChanged, this, &MainWindow::updateDataDirectories);
...
void MainWindow::setupMenuBar()
{
    ...
    QAction *actSaveMatrixData = new QAction(tr("&Save matrix data displayed in Matrix View"), this);
    fileMenu->addAction(actSaveMatrixData);
    connect(actSaveMatrixData, &QAction::triggered, this, &MainWindow::handleSaveMatrixData);

    QMenu *viewMenu = new QMenu(tr("&View"));
    menuBar()->addMenu(viewMenu);
    QAction *actShowSettingPanel = new QAction(tr("&Show settting panel"), this);
    viewMenu->addAction(actShowSettingPanel);
    connect(actShowSettingPanel, &QAction::triggered, this, &MainWindow::handleShowSettingPanel);
}
...
void MainWindow::handleOpen()
{
    ...
    const QString spatialCorrectionFilePath = brainMeshDir + "spatial_correction.csv";
    ...
    setScannedDatasetPositions(detailedMatrix->getRowsAndColsUsedInDimReduction());
...
void MainWindow::handleSaveMatrixData()
{
...
void MainWindow::updateDataDirectories(QString baseDataDir, QString brainMeshDir, QString roiDir, QString scannedDataDir, QString styleDir)
{
...
void MainWindow::setViews()
{
    ...
    informationView = new InformationView(comboBoxForList, scannedDataset, rois, detailedMatrix);
    thresholdControlView = new ThresholdControlView(detailedMatrix, style);

    connect(comparisonView, &ComparisonView::showMatrixInDetailViewsSelected, this, &MainWindow::setMatrixAsDetailedMatrix);
    connect(comparisonView, &ComparisonView::showAveMatrixInDetailViewsSelected, this, &MainWindow::setAveMatrixAsDetailedMatrix);
    connect(comparisonView, &ComparisonView::showDiffMatrixInDetailViewsSelected, this, &MainWindow::setDiffMatrixAsDetailedMatrix);
    connect(comparisonView, &ComparisonView::showSdMatrixInDetailViewsSelected, this, &MainWindow::setSdMatrixAsDetailedMatrix);
    connect(comparisonView, &ComparisonView::showMaxMinMatrixInDetailViewsSelected, this, &MainWindow::setMaxMinMatrixAsDetailedMatrix);
    connect(comparisonView, &ComparisonView::recalcMdsSelected, this, &MainWindow::setScannedDatasetPositionsWithSelectedData);
    connect(comparisonView, &ComparisonView::selectedScannedDataChanged, this, &MainWindow::updateInformationView);
    connect(thresholdControlView, &ThresholdControlView::thresholdsChanged, this, &MainWindow::updateDetailedMatrix);
    connect(matrixView, &MatrixView::detailedMatrixFilteredOutChanged, this, &MainWindow::updateViewsRelatedWithMatrixFilteredOut);
    connect(matrixView, &MatrixView::recalcMdsSelected, this, &MainWindow::setScannedDatasetPositionsWithSelectedMatrixElements);
    connect(matrixView, &MatrixView::matrixOrderChanged, this, &MainWindow::updateViewsRelatedWithMatrixOrder);
    connect(matrixView, &MatrixView::roisSubsetChanged, this, &MainWindow::updateViewsRelatedWithMatrixOrderAndRoisSubset);
    connect(matrixView, &MatrixView::hoveredRowAndColIndexChanged, this, &MainWindow::updateBrainGraphView);
    connect(brainGraphView, &BrainGraphView::hoveredRoiIndexChanged, this, &MainWindow::updateMatrixView);
    connect(informationView, &InformationView::selectedScannedDataChanged, this, &MainWindow::updateComparisonView);
    connect(informationView, &InformationView::selectedRoisChanged, this, &MainWindow::updateMatrixView);
    connect(informationView, &InformationView::matrixOrderChanged, this, &MainWindow::updateViewsRelatedWithMatrixOrder);
    connect(informationView, &InformationView::roisSubsetChanged, this, &MainWindow::updateViewsRelatedWithMatrixOrderAndRoisSubset);
    connect(comparisonViewBtn, &QPushButton::clicked, this, &MainWindow::setComparisonViewInLargeWindow);
    connect(matrixViewBtn, &QPushButton::clicked, this, &MainWindow::setMatrixViewInLargeWindow);
    connect(brainGraphViewBtn, &QPushButton::clicked, this, &MainWindow::setBrainGraphViewInLargeWindow);
}
...
{
    //const QString brainMeshDir = appDirPathToSrcDirPath() + "../data/brain_mesh/";
    const QString brainMeshFilePath = brainMeshDir + "brain_mesh.txt";
    const QString spatialCorrectionFilePath = brainMeshDir + "spatial_correction.csv";
...
{
    //const QString roisFilePath = appDirPathToSrcDirPath() + "../data/roi/rois.csv";
    const QString roisFilePath = roiDir + "rois.csv";
...
{
    //const QString scannedDataDir = appDirPathToSrcDirPath() + "../data/scanned_data/";
    scannedDataset = new ScannedDataset(scannedDataDir);
    scannedDataset->setVisTargetMatrixKey("ROI-ROI Matrix");
}

void MainWindow::setScannedDatasetPositions(QVector<QPair<int,int>> targetLinks)
{
    dimReduction->calcDimReductionPositions(scannedDataset, scannedDataset->getVisibleIndices(), targetLinks);
}

void MainWindow::setScannedDatasetPositionsAndUpdate(QVector<QPair<int,int>> targetLinks)
{
    dimReduction->calcDimReductionPositions(scannedDataset, scannedDataset->getVisibleIndices(), targetLinks);
...
void MainWindow::setScannedDatasetPositionsWithSettings(QString distCalcMethod, QString matrixKey)
{
    ...
    setScannedDatasetPositionsAndUpdate(detailedMatrix->getRowsAndColsUsedInDimReduction());
}
...
void MainWindow::setScannedDatasetFilteredOut(QMap<QString, QMap<QString, bool>> statusOfCategoryFilterCheckBoxes, QMap<QString, QPair<QString, QString>> statusOfNumberFilterOperatorsAndValues)
{
    scannedDataset->setFilteredOutWithFilterStatus(statusOfCategoryFilterCheckBoxes, statusOfNumberFilterOperatorsAndValues);
    comparisonView->update();
}
...
void MainWindow::setMatrixAsDetailedMatrix()
{
    ...
    if (selectedIndices.size() == 1) {
        ...
        detailedMatrix->initMatrix(scannedDataset->getScannedDataAt(index)->getMatrixAt(scannedDataset->getVisTargetMatrixKey()), -1.0, 1.0, -1.0, 1.0);
    }
...
{
    detailedMatrix->initMatrix(scannedDataset->genAveMatFromSelectedData(), -1.0, 1.0, -1.0, 1.0);
...
{
    detailedMatrix->initMatrix(scannedDataset->genDiffMatFromSelectedData(index1, index2), -1.0, 1.0, -1.0, 1.0);
...
{
    detailedMatrix->initMatrix(scannedDataset->genSdMatFromSelectedData(), 0.0, 0.3, 0.0, 0.3);
...
{
    detailedMatrix->initMatrix(scannedDataset->genMaxMinMatFromSelectedData(), 0.0, 2.0, 0.0, 2.0);
...
void MainWindow::updateBrainGraphView()
{
    brainGraphView->update();
}

void MainWindow::updateInformationView()
{
    informationView->updateItems();
}
...
void MainWindow::showSettingPanel()
{
    ...
    connect(settingPanel, &SettingPanel::mdsCalcMethodChanged, this, &MainWindow::setScannedDatasetPositionsWithSettings);
    connect(settingPanel, &SettingPanel::filterChanged, this, &MainWindow::setScannedDatasetFilteredOut);
    connect(settingPanel, &SettingPanel::applyFilteringClicked, this, &MainWindow::setScannedDatasetVisibleWithFiltering);
    connect(settingPanel, &SettingPanel::recalcMdsClicked, this, &MainWindow::setScannedDatasetPositionsWithSettings);
    connect(settingPanel, &SettingPanel::pointColorChanged, this, &MainWindow::setScannedDatasetColorGroups);
    connect(settingPanel, &SettingPanel::pointShapeChanged, this, &MainWindow::setScannedDatasetShapeGroups);
    connect(settingPanel, &SettingPanel::pointSizeChanged, this, &MainWindow::setScannedDatasetSizes);
}
