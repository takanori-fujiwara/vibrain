...
ScannedDataset::~ScannedDataset() {
  foreach (ScannedData *scannedData, scannedDataset) { delete scannedData; }
}
...
bool ScannedDataset::loadScannedDataset(const QString &dirPath) {
  ...
  for (int i = 0; i < fileInfos.size(); ++i) {
    ...
    if (!scannedData->getIsLoadCompleted()) {
      qWarning() << "cannot load scanned data. file path: "
  ...
  if (lineNumOfDiffNumAttr >= 0) {
    qWarning() << "some scanned data has different number of attributes. check "
                  "scanned data file: "
  ...
  foreach (ScannedData *scannedData, scannedDataset) {
    ...
    foreach (QString key, numbers.keys()) {
      if (minAndMaxOfNumbers[key].size() == 0) {
        minAndMaxOfNumbers.insert(key,
                                  QVector<float>({numbers[key], numbers[key]}));
      } else {
...
int ScannedDataset::checkAllScannedDataHaveSameNumAttr() {
  ...
  if (scannedDataset.size() > 0) {
    ...
    foreach (QString key, matrixKeys) {
      ...
      if (numRow != 0) {
        numMatricesCols.push_back(
            (int)scannedDataset[0]->getMatrixAt(key).at(0).size());
      } else {
      ...
      for (int i = 0; i < matrixKeys.size(); ++i) {
        ...
        if (numRow != numMatricesRows.at(i)) {
          ...
          for (int j = 0; j < numRow; ++j) {
            if ((int)sd->getMatrixAt(matrixKeys.at(i)).at(j).size() !=
                numMatricesCols.at(i)) {
              qWarning() << "number of cols is not equal in "
                         << matrixKeys.at(i);
              return count;
            }
          }
...
bool ScannedDataset::ifWithQStringOperator(float leftVal, QString ope,
                                           float rightVal) {
  if (ope == "<") {
...
void ScannedDataset::setFilteredOutWithFilterStatus(
  ...
  foreach (ScannedData *sd, scannedDataset) {
    ...
    foreach (QString categoryKey, categoryKeys) {
      if (!statusOfCategoryFilterCheckBoxes[categoryKey]
                                           [sd->getCategoryAt(categoryKey)]) {
    ...
    foreach (QString numberKey, numberKeys) {
      ...
      if (val != "") {
        if (!ifWithQStringOperator(sd->getNumberAt(numberKey), ope,
                                   val.toFloat())) {
          sd->setFilteredOut(true);
        }
      }
...
void ScannedDataset::setScannedDataColorGroupsFromCategoryKey(QString key) {
  if (!valuesOfCategories.contains(key)) { // to support key == "none"
    ...
    foreach (ScannedData *sd, scannedDataset) {
      ...
      if (index >= 0)
        sd->setColorGroup(index);
      else
        qWarning() << "cannot find category in setPointColorsFromCategoryKey.";
    }
  }
...
void ScannedDataset::setScannedDataSizesFromNumberKey(QString key) {
  if (!minAndMaxOfNumbers.contains(key)) { // to support key == "none"
    ...
    foreach (ScannedData *sd, scannedDataset) {
      ...
      float size = 0.005 + 0.05 * (sd->getNumberAt(key) - minMax.first()) /
                               (minMax.last() - minMax.first());
      sd->setSize(size);
    }
  }
...
QVector<ScannedData *> ScannedDataset::getScannedDataset() {
  return scannedDataset;
}
ScannedData *ScannedDataset::getScannedDataAt(int pos) {
  return scannedDataset.at(pos);
}

ScannedData *ScannedDataset::getScannedDataFromFileName(QString fileName) {
  foreach (ScannedData *sd, scannedDataset) {
    if (sd->getFileName() == fileName)
      return sd;
  }
...
QVector<int> ScannedDataset::getVisibleIndices() {
  ...
  for (int i = 0; i < scannedDataset.size(); ++i) {
    if (scannedDataset.at(i)->getVisible())
      result.push_back(i);
  }
...
QVector<int> ScannedDataset::getSelectedIndices() {
  QVector<int> result;
  for (int i = 0; i < scannedDataset.size(); ++i) {
    if (scannedDataset.at(i)->getSelected())
      result.push_back(i);
  }
...
QVector<int> ScannedDataset::getMatrixDisplayedIndices() {
  QVector<int> result;
  for (int i = 0; i < scannedDataset.size(); ++i) {
    if (scannedDataset.at(i)->getMatrixDisplayed())
      result.push_back(i);
  }
...
std::vector<std::vector<float>> ScannedDataset::genAveMatFromSelectedData() {
  ...
  int row = scannedDataset.at(selectedIndices.at(0))
                ->getMatrixAt(visTargetMatrixKey)
                .size();
  if (row == 0)
    return std::vector<std::vector<float>>();
  int col = scannedDataset.at(selectedIndices.at(0))
                ->getMatrixAt(visTargetMatrixKey)
                .at(0)
                .size();
  ...
  foreach (int index, selectedIndices) {
    std::vector<std::vector<float>> mat =
        scannedDataset.at(index)->getMatrixAt(visTargetMatrixKey);
...
ScannedDataset::genDiffMatFromSelectedData(int index1, int index2) {
  std::vector<std::vector<float>> result;

  std::vector<std::vector<float>> mat1 =
      scannedDataset.at(index1)->getMatrixAt(visTargetMatrixKey);
  std::vector<std::vector<float>> mat2 =
      scannedDataset.at(index2)->getMatrixAt(visTargetMatrixKey);
...
std::vector<std::vector<float>> ScannedDataset::genMaxMinMatFromSelectedData() {
  ...
  foreach (int index, selectedIndices) {
    ...
    for (int i = 0; i < (int)mat.size(); ++i) {
      for (int j = 0; j < (int)mat.at(i).size(); ++j) {
        float val = mat.at(i).at(j);
        if (val < mins.at(i).at(j))
          mins[i][j] = val;
        if (val > maxs.at(i).at(j))
          maxs[i][j] = val;
      }
    }
  }
...
QMap<QString, QVector<QString>> ScannedDataset::getValuesOfCategories() {
  return valuesOfCategories;
}
QMap<QString, QVector<float>> ScannedDataset::getMinAndMaxOfNumbers() {
  return minAndMaxOfNumbers;
}
QString ScannedDataset::getCategoryForColorGroups() {
  return categoryForColorGroups;
}
QString ScannedDataset::getCategoryForShapeGroups() {
  return categoryForShapeGroups;
}
...
void ScannedDataset::setVisTargetMatrixKey(QString visTargetMatrixKey) {
  this->visTargetMatrixKey = visTargetMatrixKey;
}
