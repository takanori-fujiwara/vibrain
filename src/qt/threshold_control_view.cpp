...
ThresholdControlView::ThresholdControlView(DetailedMatrix* detailedMatrix, Style *style, QWidget *parent)
{
...
{
    glClearColor(style->bgColorForControls.redF(), style->bgColorForControls.greenF(), style->bgColorForControls.blueF(), style->bgColorForControls.alphaF());
...
void ThresholdControlView::drawScale(float x, float y, float w, float h, int numLabelsBetweenMinMax)
{
...
void ThresholdControlView::drawSlider(float x, float y, float w, float h)
{
    ...
    // draw slider points
    foreach (float basePos, QVector<float>({lowerThresValPos, upperThresValPos})) {
        std::vector<GLfloat> pos = std::vector<GLfloat>({basePos, y + h,
                                             (float)(basePos - 0.5 * sliderPointSizeX), (float)(y + h - 0.5 * h),
                                             (float)(basePos - 0.5 * sliderPointSizeX), (float)(y + h - 1.0 * h),
                                             (float)(basePos + 0.5 * sliderPointSizeX), (float)(y + h - 1.0 * h),
                                             (float)(basePos + 0.5 * sliderPointSizeX), (float)(y + h - 0.5 * h)});
...
void ThresholdControlView::paintValueLabels(int startX, int startY, int width, int fontSize, int numLabelsBetweenMinMax)
{
    ...
    // label for min
    painter.drawText(startX, startY,
                     fontW * QString::number(minVal).size(), fontH,
                     Qt::AlignTop | Qt::AlignLeft, QString::number(minVal));
    // label for max
    painter.drawText(startX + width - fontW * QString::number(maxVal).size(), startY,
                     fontW * QString::number(maxVal).size(), fontH,
...
bool ThresholdControlView::isInsidePolygon(QPointF point, std::vector<GLfloat> selectionAreaGlBuffer)
{
    bool isInside = false;
...
void ThresholdControlView::changeLowerThresVal(QPointF mousePosF)
{
    ...
    if (mousePosF.x() < sliderStartX) {
        ...
        lowerThresVal = ((mousePosF.x() - sliderStartX) / sliderWidth) * (maxVal - minVal) + minVal;
    }

    if (lowerThresVal > upperThresVal) lowerThresVal = upperThresVal;
...
void ThresholdControlView::changeUpperThresVal(QPointF mousePosF)
{
    ...
    if (mousePosF.x() < sliderStartX) {
        ...
        upperThresVal = ((mousePosF.x() - sliderStartX) / sliderWidth) * (maxVal - minVal) + minVal;
    }

    if (upperThresVal < lowerThresVal) upperThresVal = lowerThresVal;
...
void ThresholdControlView::showContextMenu(const QPoint &pos)
{
    ...
    connect(&actShowThresholdSettingDialog, &QAction::triggered, this, &ThresholdControlView::showThresholdSettingDialog);
...
{
    ThresholdSettingDialog *thresholdSettingDialog = new ThresholdSettingDialog(detailedMatrix);
    connect(thresholdSettingDialog, &ThresholdSettingDialog::thresholdsChanged, this, &ThresholdControlView::changeThresholdsFromDialog);
