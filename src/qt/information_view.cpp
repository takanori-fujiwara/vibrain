...
InformationView::InformationView(QComboBox *comboBox, ScannedDataset *scannedDataset, Rois* rois, DetailedMatrix *detailedMatrix, QWidget *parent)
{   
    ...
    connect(this->comboBox, &QComboBox::currentTextChanged, this, &InformationView::updateItems);
    connect(this, &InformationView::itemSelectionChanged, this, &InformationView::handleSelectedItem);
    //connect(this->model(), &QAbstractItemModel::rowsMoved, this, &InformationView::handleDropEvent);
}
    ...
    if (comboBoxCurrentText == "Displayed Scanned Data") {
        foreach (ScannedData* scannedData, scannedDataset->getScannedDataset()) {
            if (scannedData->getVisible()) {
                ...
                if (scannedData->getSelected()) selectedLists.push_back(true);
                else selectedLists.push_back(false);
            }
        }
        ...
        if (scannedDataset->getVisTargetMatrixKey() == "ROI-ROI Matrix") {
            foreach (int index, rois->getOrder()) {
                if (index >= 0) lists.push_back(rois->getRoiAt(index)->getName());
            }
        } else {
            // TODO: support later
        ...
        if (scannedDataset->getVisTargetMatrixKey() == "ROI-ROI Matrix") {
            ...
            for (int i = 0; i < degrees.size(); ++i) {
                if (degrees.at(i) > degreeThres) {
                    if (rois->getNumOfRois() > i) lists.push_back(rois->getRoiAt(i)->getName());
                }
            }
        }
        ...
        for (int i = 0; i < detailedMatrix->getNumOfRows(); ++i) {
            for (int j = i; j < detailedMatrix->getNumOfCols(); ++j) {
                if (detailedMatrix->getVisibleAt(i, j) &&
                    ...
                    lists.push_back(rois->getRoiAt(i)->getName() + " - " +rois->getRoiAt(j)->getName());
                }
            }
        }
    }
...
void InformationView::setListsSelected(QList<bool> selectedLists)
{
    ...
    {
        if (selectedLists.at(i)) this->item(i)->setSelected(true);
        else this->item(i)->setSelected(false);
    }
}

void InformationView::updateItems()
{
    if (comboBox->currentText() == "Order of Matrix") setDragDropMode(QAbstractItemView::InternalMove);
...
void InformationView::handleSelectedItem()
{
    if (comboBox->currentText() == "Displayed Scanned Data") {
        for(int i = 0; i < this->count(); ++i)
        {
            ...
            if (item->isSelected()) scannedDataset->getScannedDataFromFileName(item->text())->setSelected(true);
            else scannedDataset->getScannedDataFromFileName(item->text())->setSelected(false);
        }
...
void InformationView::dropEvent(QDropEvent *event)
{
    ...
    foreach (int index, selectedIndices) {
        if (index < insertPosition) insertPosition--;
    }
...
void InformationView::showContextMenu(const QPoint &pos)
{
    ...
    connect(&actSettingSubsetRois, &QAction::triggered, this, &InformationView::showSettingSubsetRoisDialog);
    connect(&actResetRoisOrder, &QAction::triggered, this, &InformationView::resetRoisOrder);
...
{
    SubsetRoisSettingDialog *subsetRoisSettingDialog = new SubsetRoisSettingDialog(rois);
    connect(subsetRoisSettingDialog, &SubsetRoisSettingDialog::roisOrderChanged, this, &InformationView::changeRoisOrderFromDialog);
    subsetRoisSettingDialog->show();
}

void InformationView::changeRoisOrderFromDialog()
{
...
void InformationView::setScannedDataset(ScannedDataset* scannedDataset) { this->scannedDataset = scannedDataset; }
