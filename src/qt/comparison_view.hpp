...
private:
  ...
  void drawRadialColCircle(float x, float y, float radius, QColor colorCenter,
                           QColor colorPeri);
  void drawPoint(int shapeType, float x, float y, float size, QColor fillColor,
  ...
  void drawLineQuad(float x, float y, float w, float h, float width,
                    QColor color);
  void drawMatrix(QString matrixKey, std::vector<std::vector<float>> mat,
                  float x, float y, float size, QVector<int> orderOfMatrix,
                  QVector<QColor> colormap, QColor strokeColor,
                  float strokeWidth = 1.0);
  void drawLineFromMatrixToPoint(QPointF startPos, QPointF endPos, float width,
                                 QColor color);

  QPolygonF genLegendShapePoints(int x, int y, int size, int shapeType);
  QPointF qtPosToGlPos(QPoint qtPos);
  bool isInsidePolygon(QPointF point,
                       std::vector<GLfloat> selectionAreaGlBuffer);

  std::vector<GLfloat> genCirclePoints(float cx, float cy, float r,
                                       int numSplit = 6);
  void selectElement(std::vector<GLfloat> selectionAreaGlBuffer);
  void hoverElement(std::vector<GLfloat> selectionAreaGlBuffer);
  void
  selectMovingMatrixScannedDataId(std::vector<GLfloat> selectionAreaGlBuffer);
